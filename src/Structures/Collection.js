...
class Collection extends Abstract {
    ...
    /**
     * @returns {Array} An array of each model's errors.
     */
    get errors() {
     ...
     */
    filter(predicate) {
        return new (this.constructor)(this.where(predicate));
    }
    ...
    getModelsFromResponse(response) {
        let models = this.getResponseData(response);
    ...
    onSaveSuccess(response) {
        ...
        // These are the models of this collection that are in a saving state,
        // as set by `onSave`. It's possible that some models were not included
        // in the request, so we should not concern ourselves with them here.
        ...
        if (_.isNil(saved)) {
            ...
            _.each(saved, (data, index) => {
                saving[index].onSaveSuccess({data, status: 200});
            });
        }
    ...
    applyValidationErrorArray(errors) {
        ...
        if (errors.length !== models.length) {
            throw new ResponseError('Array of errors must equal the number of model', response);
        }
        ...
        _.each(models, (model, index) => {
            model.onSaveValidationFailure(_.get(errors, index, {}));
        });
    }
     ...
     * @param {Object} response
     */
    onSaveValidationFailure(response) {
        let errors = this.getValidationErrors(response);
    ...
    onSaveFailure(error, response) {
        if (this.isValidationError(response)) {
            this.onSaveValidationFailure(response);
    ...
    onFetchSuccess(response) {
        ...
        // is considered an exception that indicates an unexpected state.
        if ( ! _.isArray(models)) {
            throw new ResponseError('Expected an array of models in fetch response', response);
        }
    ...
    onDeleteFailure(error, response) {
        ...
        _.each(this.getDeletingModels(), (model) => {
            model.onDeleteFailure(error, response);
        });
    }
    ...
    onSave() {
        ...
        // Exclude all models that return `false` on save.
        let models = _.filter(this.models, (model) => {
            return model.onSave(this) !== false;
        });

        // Don't save if there are no models to save.
        if (_.isEmpty(models)) {
            return false;
        }

        // Abort if any of the save event listeners returned false.
        if ( ! this.emit('save', {models})) {
            return false;
        }
    ...
    onDelete() {
        ...
        let models = _.filter(this.models, (model) => {
            return model.onDelete(this) !== false;
        });
